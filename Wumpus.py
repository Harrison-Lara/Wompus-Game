...
class WumpusWorld(object):
    '''User interactions and movement, warning signals and dangers become active'''
    ...
    def wumpusAlive(self, Map, x = 0, y = 0, s = "A"):
        '''Check to see if Wumpus is alive'''
        self.Map = Map
        self.name = s
        self.x = x
        self.y = y
    ...
    def playerAlive(self, Map, x = 0, y = 0, s = "A"):
    ...
    def playerHasGold(self, Map, x = 0, y = 0, s = "A"):
        ''' Check and see if gold is in hands or not'''
        if self.playerHasGold
    
    def playerHasArrow(self):
        ''' See if player has an arrow still or not'''
        if self.playerHasArrow

    def playerMoves(self):
        ''' Player movement'''
      if not self.movmoveed:
            location = self.OffMapError(int)
            if location:
...
class __init__(object):
    ''' Create movement directions, get gold, make exit, shoot arrow, warnings''' 
    ...
    def feelBreeze(self):
        ''' Give warning when near pit'''
            # check map
        hasBreeze()
        print('There is a pit near by')
        
    def smellStench(self):
        ''' Give warning when near wumpus'''
        # check map
        hasStench()
        print('There is a wumpus near by')
        
    def seeGlint(self):
        ''' Gold is in same cell'''
        # check map
        hasGold()
        print('Here is the gold!')
